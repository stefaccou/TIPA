        ...
        try:
            ...
            for distance_type in distance_types:
                for lang in eval_languages:
                    ...
                    adapter_path = (
                        f"./trained_adapters/typological/{eval_language}/{distance_type}{limit_p}_extended_{lang}"
                    )
                    weights[distance_type] = {}
                    if os.path.exists(adapter_path):
                        print("Adapter already exists, loading instead")
                        ...
                        to_load["Added_adapter"] = lang
                        weights[distance_type] = typological_approximation(
                            target_glot, get_glots(to_load), distance_type, custom_args.limit
                        )
                        model.load_adapter(f"./trained_adapters/{lang}", load_as=lang)
                        merge_loaded_adapters(
                            model, merge_adapter_name=adapter_name, weights=weights[distance_type], delete_other=False
                        )
                        # save this adapter
                ...
                model.load_adapter("./trained_adapters/sr", load_as="sr")
                to_load["eu"] = "eu"
                to_load["sr"] = "sr"
                weights[distance_type] = typological_approximation(
                    target_glot, get_glots(to_load), distance_type, custom_args.limit
                ...
                merge_loaded_adapters(
                    model, merge_adapter_name="extended_both", weights=weights[distance_type], delete_other=False
                )
                model.load_adapter("./trained_adapters/ner", load_as="ner")
                print(
                ...
                # delete the adapter for further iterations
                model.delete_adapter("ner")
                model.delete_adapter("eu")
                model.delete_adapter("sr")
                del to_load["eu"]
                del to_load["sr"]
