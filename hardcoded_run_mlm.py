...
class DataTrainingArguments:
    ...
    # train_files: Optional[str] = field(default=None, metadata={"help": "The input training data files (a text file)."})
...
def main():
    ...
    def alternating_iterator(ds1, ds2, batch_size):
        # Create iterators for both datasets
        iter1 = iter(ds1)
        iter2 = iter(ds2)
        while True:
            # Take a batch from each iterator
            batch1 = list(islice(iter1, batch_size))
            batch2 = list(islice(iter2, batch_size))
            # Stop if both are exhausted
            if not batch1 and not batch2:
                break
            # Yield the batch from ds1 if available
            if batch1:
                for example in batch1:
                    yield example
            # Then yield the batch from ds2 if available
            if batch2:
                for example in batch2:
                    yield example
