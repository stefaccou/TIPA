...
def main(submit_arguments):
    ...
    class CustomArguments:
        ...
        generate: Optional[bool] = field(
            default=False,
            metadata={
                "help": ("Whether to generate sample text after evaluation. Default False, it will not generate text.")
            },
        )
    ...
    if custom_args.generate:

        def generate_text(prompt, max_new_tokens=50):
            inputs = tokenizer(prompt, return_tensors="pt")
            with torch.no_grad():
                outputs = model.generate(
                    **inputs, max_new_tokens=max_new_tokens, do_sample=True, top_k=50, top_p=0.95, temperature=0.7
                )
