...
def find_master(par="TIPA"):
    """
    Search for a parent directory named 'TIPA'.

    Returns:
        Path: The Path object corresponding to the 'TIPA' directory.

    Raises:
        FileNotFoundError: If no parent directory named 'TIPA' is found.
    """
    current = Path(__file__).resolve()
    for parent in [current] + list(current.parents):
        if parent.name == par:
            return parent
    raise FileNotFoundError("Could not find a parent directory named 'TIPA'")
